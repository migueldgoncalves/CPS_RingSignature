...
public class GroupMember {
    ...
    public void sign(String message) {
        try {
            ...
            // Step 3 - Get random values for all other group members and cipher them with their public keys

            HashMap<Integer, byte[]> randomValues = new HashMap<>();
            byte[] randomBytesForOtherMembers = new byte[256];
            ...
            for(int i=1; i<=randomValues.size(); i++) {
                if (randomValues.get(i) != null) {
                    ...
                    cipher.init(Cipher.ENCRYPT_MODE, Manager.publicKeys.get(i));

                    byte[] input = randomValues.get(i);
                    cipher.update(input);

                    byte[] cipherValue = cipher.doFinal();
                    System.out.println(new String(cipherValue, StandardCharsets.UTF_8));
            ...
            for(int i=1; i<=cipheredRandomValues.size(); i++) {
                ...
                for(int j=0; j<256; j++) {
                    toDecipher[j] = (byte) (decipheredValue[j] ^ cipheredRandomValues.get(i)[j]);
                }
            }
            //One cycle remaining
            byte[] cipheredRandomValueForThisMember = new byte[256];
            byte[] decipheredValue = decipher(toDecipher, messageDigest);
            for(int j=0; j<256; j++) {
                cipheredRandomValueForThisMember[j] = (byte) (decipheredValue[j] ^ randomBytes[j]);
            }

            System.out.println("Step 4 completed");

            // Step 5 - Decipher random value for this member using its private key

            Cipher decipher = Cipher.getInstance("RSA/ECB/PKCS1Padding");
            decipher.init(Cipher.DECRYPT_MODE, Manager.privateKeys.get(id));

            //byte[] randomValueForThisMember = decipher.doFinal(cipheredRandomValueForThisMember);
            byte[] randomValueForThisMember = cipheredRandomValueForThisMember;
            randomValues.put(id, randomValueForThisMember);

            System.out.println(new String(randomValueForThisMember));
            ...
            signedMessage.setRandomValues(randomValues);
    ...
    public byte[] decipher(byte[] toDecipher, byte[] key) {
        try {
            byte[] decipheredBytes = new byte[256];
            for(int i=0; i<256; i++) {
